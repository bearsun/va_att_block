function v_run(debug)
%V_RUN fmri run for visual attention task
%   Visual endogenous cue at the center, visual target at sides
%   Participants have to indicate the side of visual target
...
path_data = [pwd, '/data/data-', subj, '-v'];
...
% atars = [buffer_l, buffer_r]; % see make_seq
% 
% % warm up
...
img = imread('VV_bg.bmp') * 255;
bg = Screen('MakeTexture', mainwin, img);
img = imread('VV_l_target.bmp') * 255;
l_target = Screen('MakeTexture', mainwin, img);
img = imread('VV_r_target.bmp') * 255;
r_target = Screen('MakeTexture', mainwin, img);
vtars = [l_target, r_target]; % see make_seq

img = imread('VV_lcue.bmp') * 255;
lcue = Screen('MakeTexture', mainwin, img);
img = imread('VV_rcue.bmp') * 255;
rcue = Screen('MakeTexture', mainwin, img);
img = imread('VV_long_cue.bmp') * 255;
longcue = Screen('MakeTexture', mainwin, img);
img = imread('VV_short_cue.bmp') * 255;
shortcue = Screen('MakeTexture', mainwin, img);
img = imread('VV_neutral_cue.bmp') *255;
...
for iblock = 1:nblocks
    ...
    for itrial = 1:ntpb
        ...
        Screen('DrawTexture', mainwin, vtars(dtar));
