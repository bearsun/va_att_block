function va_run(debug)
%VA_RUN fmri run for divided attention task (visual + auditory)
%   Visual endogenous cue at the center, visual/auditory target at sides
%   Participants have to indicate whether the v/a targets are congruent or
%   incongruent
...
path_data = [pwd, '/data/data-', subj, '-va'];
outfile = fopen(path_data, 'w');

fprintf(outfile, ...
    '%s\t %s\t %s\t %s\t %s\t %s\t %s\t %s\t %s\t %s\t %s\t %s\n', ...
    'block', 'trial', 'cue', 'soa', 'aud_tar', 'vis_tar', 'keypressed', ...
...
for iblock = 1:nblocks
    ...
    for itrial = 1:ntpb
        ...
        Screen('DrawTexture', mainwin, vtars(vtar));
